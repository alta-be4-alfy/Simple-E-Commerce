...
func GetShoppingCarts(id int) (interface{}, error) {
	...
	query := config.DB.Table("shopping_carts").Select("shopping_carts.qty, shopping_carts.price, products.product_name, users.user_name, users.id").Joins("join products on products.id = shopping_carts.products_id").Joins("join users on users.id = shopping_carts.users_id").Where("shopping_carts.users_id = ?", id).Find(&shoppingCart)
	if query.Error != nil {
